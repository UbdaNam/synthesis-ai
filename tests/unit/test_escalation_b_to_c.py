...
class _LowLayout(BaseExtractionStrategy):
    name = "layout_aware"

    def extract(self, context: ExtractionContext) -> StrategyResult:
        return StrategyResult("layout_aware", 0.3, None, 0.01)


class _GoodVision(BaseExtractionStrategy):
    name = "vision"

    def extract(self, context: ExtractionContext) -> StrategyResult:
        return StrategyResult("vision", 0.95, make_doc(strategy="vision"), 0.03, usage_tokens=500)
...
def test_escalation_from_b_to_c(temp_config, workspace_tmp):
    router = ExtractionRouter(
        config_path=str(temp_config),
        ledger_path=str(workspace_tmp / "ledger.jsonl"),
        strategy_registry={"fast_text": _LowFast(), "layout_aware": _LowLayout(), "vision": _GoodVision()},
    )
    out = router.extract_node(_state())
    assert out.extracted_document is not None
    assert out.extracted_document.strategy_used == "vision"
    assert len(out.extraction_attempts) == 3
