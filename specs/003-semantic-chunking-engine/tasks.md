...
- [X] T001 Create Stage 3 module scaffolding in `src/agents/chunker.py`, `src/models/ldu.py`, `src/models/chunk_relationship.py`, `src/chunking/engine.py`, `src/chunking/validator.py`, `src/chunking/reference_resolver.py`, `src/chunking/token_counter.py`, and `src/chunking/hash_generator.py`
- [X] T002 [P] Add Stage 3 configuration placeholders under `chunking` in `rubric/extraction_rules.yaml`
- [X] T003 [P] Create Stage 3 test skeletons in `tests/unit/test_ldu_schema.py`, `tests/unit/test_chunk_relationship_schema.py`, and `tests/integration/test_chunker_end_to_end.py`
...
- [X] T004 Define the `LDU` Pydantic schema in `src/models/ldu.py`
- [X] T005 Define the `ChunkRelationship` Pydantic schema in `src/models/chunk_relationship.py`
- [X] T006 Extend `GraphState` for Stage 3 outputs in `src/models/graph_state.py`
- [X] T007 [P] Implement deterministic token counting in `src/chunking/token_counter.py`
- [X] T008 [P] Implement stable LDU content hash generation in `src/chunking/hash_generator.py`
- [X] T009 [P] Implement cross-reference detection and resolution primitives in `src/chunking/reference_resolver.py`
- [X] T010 Implement `ChunkValidator` invariants and failure categories in `src/chunking/validator.py`
- [X] T011 Externalize Stage 3 thresholds and rule settings in `rubric/extraction_rules.yaml`
- [X] T012 Add foundational schema and helper tests in `tests/unit/test_ldu_schema.py`, `tests/unit/test_chunk_relationship_schema.py`, `tests/unit/test_token_counter.py`, and `tests/unit/test_hash_generator.py`
...
- [X] T013 [P] [US1] Add unit test for Stage 3 entrypoint state handling in `tests/unit/test_chunker_agent.py`
- [X] T014 [P] [US1] Add unit test for text chunk construction from `ExtractedDocument` in `tests/unit/test_chunking_text_blocks.py`
- [X] T015 [P] [US1] Add unit test for heading and section metadata propagation in `tests/unit/test_section_propagation.py`
- [X] T016 [P] [US1] Add integration test for `ExtractedDocument -> ChunkingEngine -> validated List[LDU]` in `tests/integration/test_chunker_end_to_end.py`

### Implementation for User Story 1

- [X] T017 [US1] Implement `ChunkingEngine` reading-order traversal over the existing `ExtractedDocument` schema in `src/chunking/engine.py`
- [X] T018 [US1] Implement heading and section context tracking in `src/chunking/engine.py`
- [X] T019 [US1] Implement text chunk construction in `src/chunking/engine.py`
- [X] T020 [US1] Implement the Semantic Chunking Engine entrypoint in `src/agents/chunker.py`
- [X] T021 [US1] Wire Stage 3 node state emission through `GraphState` in `src/agents/chunker.py` and `src/models/graph_state.py`
- [X] T022 [US1] Integrate Stage 3 into the LangGraph flow in `src/graph/graph.py`
...
- [X] T023 [P] [US2] Add unit test for structured table chunking with repeated headers in `tests/unit/test_table_chunking.py`
- [X] T024 [P] [US2] Add unit test for figure chunk caption metadata attachment in `tests/unit/test_figure_chunking.py`
- [X] T025 [P] [US2] Add unit test for numbered list grouping and controlled splitting in `tests/unit/test_numbered_list_chunking.py`
- [X] T026 [P] [US2] Add unit test for cross-reference detection and resolution in `tests/unit/test_reference_resolution.py`
- [X] T027 [P] [US2] Add integration test for mixed-content structural preservation in `tests/integration/test_chunker_mixed_content.py`

### Implementation for User Story 2

- [X] T028 [US2] Implement structured table chunk construction with header preservation in `src/chunking/engine.py`
- [X] T029 [US2] Implement figure chunk construction with caption metadata in `src/chunking/engine.py`
- [X] T030 [US2] Implement numbered list grouping and controlled splitting in `src/chunking/engine.py`
- [X] T031 [US2] Implement cross-reference resolution and relationship attachment in `src/chunking/engine.py` and `src/chunking/reference_resolver.py`
- [X] T032 [US2] Implement `belongs_to_section` and `follows` relationship emission in `src/chunking/engine.py`
...
- [X] T033 [P] [US3] Add unit tests for the five chunking-rule validations in `tests/unit/test_chunk_validator_rules.py`
- [X] T034 [P] [US3] Add unit tests for validator failure cases in `tests/unit/test_chunk_validator_failures.py`
- [X] T035 [P] [US3] Add integration test for fail-closed chunking behavior in `tests/integration/test_chunker_fail_closed.py`

### Implementation for User Story 3

- [X] T036 [US3] Implement explicit enforcement of all five chunking rules in `src/chunking/validator.py`
- [X] T037 [US3] Implement required-field, relationship-consistency, and provenance validation in `src/chunking/validator.py`
- [X] T038 [US3] Add chunking audit metadata and structured error emission in `src/agents/chunker.py`
- [X] T039 [US3] Add persisted Stage 3 audit settings and rule-version usage in `rubric/extraction_rules.yaml` and `src/agents/chunker.py`
...
- [X] T040 [P] Document Stage 3 chunking configuration and output expectations in `README.md`
- [X] T041 Reconcile Stage 3 quickstart scenarios with final implementation expectations in `specs/003-semantic-chunking-engine/quickstart.md`
- [X] T042 Run and stabilize the full Stage 3 test suite in `tests/unit/` and `tests/integration/`
